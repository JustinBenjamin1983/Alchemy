...
FUNCTION_APP="alchemy-aishop-func-app-test-docker"
...
ZIP_SIZE_BYTES=$(stat --format=%s "$OUTPUT_ZIP")
...
EXPIRY_TIME=$(date -u -d "+${SAS_EXPIRY_DAYS} days" '+%Y-%m-%dT%H:%MZ')
SAS_URL=$(az storage blob generate-sas --auth-mode login --as-user \
  --account-name $STORAGE_ACCOUNT \
  --container-name $CONTAINER_NAME \
  --name $OUTPUT_ZIP \
  --permissions r \
