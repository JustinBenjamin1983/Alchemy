...
# Supports both Azure Blob Storage (production) and Firebase Storage (dev mode)
...
if _is_dev_mode():
    logging.info("🔧 [blob_storage] DEV MODE - Using Firebase Storage adapter")
    from .dev_adapters.firebase_storage import (
        save_opinion_draft_to_blob as _dev_save_draft,
        get_opinion_draft_from_blob as _dev_get_draft,
        delete_opinion_draft_from_blob as _dev_delete_draft,
        upload_file_to_storage as _dev_upload,
        download_file_from_storage as _dev_download,
        generate_signed_url as _dev_signed_url
    )
else:
...
def save_opinion_draft_to_blob(draft_id: str, draft_data: dict) -> bool:
    ...
    """
    # Use Firebase in dev mode
    if _is_dev_mode():
        return _dev_save_draft(draft_id, draft_data)
...
def get_opinion_draft_from_blob(draft_id: str) -> dict:
    ...
    # Use Firebase in dev mode
    if _is_dev_mode():
        return _dev_get_draft(draft_id)
...
def delete_opinion_draft_from_blob(draft_id: str) -> bool:
    ...
    # Use Firebase in dev mode
    if _is_dev_mode():
        return _dev_delete_draft(draft_id)
