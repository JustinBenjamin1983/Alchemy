...
# Supports both Azure Table Storage (production) and Firebase Firestore (dev mode)
import uuid
import json
import os
import logging
...
# Conditionally import based on mode
if _is_dev_mode():
    logging.info("🔧 [table_storage] DEV MODE - Using Firebase Firestore adapter")
    from .dev_adapters.firebase_db import (
        get as _dev_get,
        save as _dev_save,
        get_user_info as _dev_get_user_info,
        save_user_info as _dev_save_user_info,
        get_opinions_settings as _dev_get_opinions_settings,
        save_opinion_settings as _dev_save_opinion_settings,
        get_dds as _dev_get_dds,
        save_dds as _dev_save_dds,
        get_dd_mapping as _dev_get_dd_mapping,
        save_dd_mapping as _dev_save_dd_mapping,
        get_docs_history as _dev_get_docs_history,
        save_docs_history as _dev_save_docs_history,
        get_opinion_draft as _dev_get_opinion_draft,
        save_opinion_draft as _dev_save_opinion_draft,
        delete_opinion_draft as _dev_delete_opinion_draft
    )
    from .dev_adapters.firebase_storage import (
        save_opinion_draft_to_blob,
        get_opinion_draft_from_blob,
        delete_opinion_draft_from_blob
    )
else:
...
# EXISTING FUNCTIONS - Updated to use dev adapters when in dev mode
def get_user_info(email: str):
    if _is_dev_mode():
        return _dev_get_user_info(email)
...
def save_user_info(row_key: str, data: dict, allowed_keys: list = None, add_to_item: str = None):
    if _is_dev_mode():
        return _dev_save_user_info(row_key, data, allowed_keys, add_to_item)
...
def get_opinions_settings():
    if _is_dev_mode():
        return _dev_get_opinions_settings()
...
def save_opinion_settings(data: dict, allowed_keys: list, add_to_item: str = None):
    if _is_dev_mode():
        return _dev_save_opinion_settings(data, allowed_keys, add_to_item)
...
def get_dds(row_key: str):
    if _is_dev_mode():
        return _dev_get_dds(row_key)
    return get(row_key, os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DDs_TABLE_NAME"], "dds")

def save_dds(row_key: str, data: dict, allowed_keys: list = None, add_to_item: str = None):
    if _is_dev_mode():
        return _dev_save_dds(row_key, data, allowed_keys, add_to_item)
    return save(row_key, data, allowed_keys, os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DDs_TABLE_NAME"], "dds", add_to_item)

def get_dd_mapping():
    if _is_dev_mode():
        return _dev_get_dd_mapping()
    try:
        return get("all", os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DD_MAPPING_TABLE_NAME"], "dd_mapping")
    except Exception:
        return None

def save_dd_mapping(data: dict, allowed_keys: list = None, add_to_item: str = None):
    if _is_dev_mode():
        return _dev_save_dd_mapping(data, allowed_keys, add_to_item)
    return save("all", data, allowed_keys, os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DD_MAPPING_TABLE_NAME"], "dd_mapping", add_to_item)

def get_docs_history(row_key: str):
    if _is_dev_mode():
        return _dev_get_docs_history(row_key)
    return get(row_key, os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DDs_DOCS_HISTORY_TABLE_NAME"], "docs_history")

def save_docs_history(row_key: str, data: dict, allowed_keys: list = None, add_to_item: str = None):
    if _is_dev_mode():
        return _dev_save_docs_history(row_key, data, allowed_keys, add_to_item)
    return save(row_key, data, allowed_keys, os.environ["DD_DOCS_BLOB_STORAGE_CONNECTION_STRING"], os.environ["DDs_DOCS_HISTORY_TABLE_NAME"], "docs_history", add_to_item)